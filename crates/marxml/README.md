...
> [!WARNING]
> **Pre-release · under active development.** `marxml` is in the `0.0.x` placeholder phase — the name on crates.io is reserved, but the working API lands in `0.1.0`. APIs, types, and selector grammar will change without notice until then. Don't depend on it in production yet.
