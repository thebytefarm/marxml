...
Both ship as **one product**: the crate and the npm package always share the same version (enforced by `scripts/check-versions.sh` in CI). The library parses markdown documents that embed XML-shaped tags, lets you query them with CSS-subset selectors, and lets you mutate them with byte-preserving string splices. Background: see [`README.md`](./README.md), [`docs/ARCHITECTURE.md`](./docs/ARCHITECTURE.md), and [`docs/dsl/selectors.md`](./docs/dsl/selectors.md).
...
The Rust API mirrors [`scraper`](https://github.com/rust-scraper/scraper). The Node API mirrors the Rust API. Divergence between the two surfaces is a smell.
...
- **Conventional Commits.** Format: `type(scope): subject`. Knope reads these too. See [Git](#git).
...
- **New regex for tokenization.** The tokenizer is a hand-rolled state machine. Regex breaks on same-tag nesting and on the byte-offset bookkeeping the mutators rely on. Regex is fine for selector attr-value matching and schema validation (where it's already used).
...
- **Selector grammar changes.** Keep [`docs/dsl/selectors.md`](docs/dsl/selectors.md) as the source of truth.
...
├── .github/workflows/                    # ci.yml, bench.yml, release-pr.yml, release.yml
...
5. **Docs in sync.** Public API changes update doc comments. Selector changes update [`docs/dsl/selectors.md`](docs/dsl/selectors.md). Architecture changes update [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).
...
**A new selector pseudo-class or combinator:** Update `crates/marxml/src/selector/` (parser + matcher), add insta/proptest cases under `crates/marxml/tests/`, then update [`docs/dsl/selectors.md`](docs/dsl/selectors.md). The Node side passes selectors through as strings. No mirror needed.
...
- **Mutators return `String`, not `Document`.** Chained mutation re-parses. That keeps every public mutator round-trip-safe in isolation. Don't add a "fast path" that operates on a held tree without reading [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) first.
...
- **Selectors require quoted attribute values.** `task[id="t1"]` works; `task[id=t1]` errors. Grammar is in [`docs/dsl/selectors.md`](docs/dsl/selectors.md).
